define(["jquery"],function($){
    ...
    var GesturePasswd= function (element, options) {
        ...
        var that=this;
        this.pr=options.pointRadii;  // 圆圈半径
        this.rr=options.roundRadii; // 中心圆点半径
        this.o=options.space;       // 圆圈间距
        this.color=options.color;   // 主颜色：圆边、圆心
        //ȫ����ʽ
        this.$element.css({
            "position":"relation",
            "width":this.options.width,         // 容器宽
            "height":this.options.height,       // 容器高
            "background-color":options.backgroundColor,  // 容器背景颜色
            "overflow":"hidden",
            "cursor":"default"
        });
        ...
        //ѡ�����淶
        if(! $(element).attr("id"))    // 没有id，添加随机id
