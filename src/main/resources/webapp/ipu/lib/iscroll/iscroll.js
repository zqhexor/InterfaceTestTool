    ...
    iScroll = function (el, options) {
      ...
      that.options = {
        ...
        onBeforeScrollMove: function (e) {
          e.preventDefault();  // 在touchstart事件中阻止默认行为，会妨碍复制等行为
        },
        onScrollMove: null,
        ...
        if (hasTouch) {
          if (that.doubleTapTimer && that.options.zoom) {
            ...
            that.doubleTapTimer = setTimeout(function () {
              ...
              // 调用多次时，则会同时生成两次click事件，所以成生事件后，标记一下，然后后面有标记时则不再生成click事件
              if (target.tagName != 'SELECT' && target.tagName != 'INPUT' && target.tagName != 'TEXTAREA' && !e.tiggerClick) {
                e.tiggerClick = true;
